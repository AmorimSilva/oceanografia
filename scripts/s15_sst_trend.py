...
file_name = download_OCEAN('SST-T', date, DADOS_DIR)
...
cmap = mcolors.LinearSegmentedColormap.from_list('sst_trend', colors)

# Create figure
fig, ax = plt.subplots(figsize=FIG_SIZE,
                        subplot_kw={'projection': ccrs.PlateCarree()})

img = ax.imshow(data_subset, vmin=-3.0, vmax=3.0, cmap=cmap,
...
# Vertical colorbar
plt.colorbar(img, label='Taxa de Mudança da TSM nos Últimos 7 dias (°C)', extend='both',
             orientation='horizontal', pad=0.06, fraction=0.046, aspect=35)

add_branding(fig, f'Taxa de Mudança da TSM nos Últimos 7 dias\nNOAA CRW - {date_formatted}', ax)

save_dir = os.path.join(OUTPUT_DIR, 'TSM_Tendencia')
os.makedirs(save_dir, exist_ok=True)
plt.savefig(os.path.join(save_dir, f'TSM_Tendencia_{date}.png'), bbox_inches='tight', dpi=FIG_DPI)
plt.show()

ds.close()
print(f'TSM_Tendencia_{date}.png saved.')
